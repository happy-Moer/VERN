...
def ssim_loss(output, bicubic_image):
    loss_fn = SSIM()
...
for i_epoch in range(c.epochs):
    ...
    for i_batch, data in enumerate(datasets.trainloader):
        ...
        s_loss = ssim_loss(steg_img.cuda(), cover.cuda())

        # print(s_loss)


        total_loss = c.lamda_reconstruction * r_loss + c.lamda_guide * g_loss + c.lamda_low_frequency * l_loss + 10 * s_loss
        total_loss.backward()
        optim.step()
        optim.zero_grad()

    print('epoch',i_epoch,':',total_loss.item())
    ...
    if i_epoch % c.val_freq == 0:
        with torch.no_grad():
            ...
            print("PSNR_S, average psnr:", np.mean(psnr_s), i_epoch)
            print("PSNR_C, average psnr:", np.mean(psnr_c), i_epoch)
